...
const Navbar = () => {
  ...
  return (
    <>
      <div className='fixed top-0 right-0 w-11/12 -z10 translate-y-[-80%]'>
      ...
      <nav className='w-full fixed px-5 lg:px-8 x1:px-[8%] py-4 flex items-center justify-between z-50'>
        <a href='#top'>
          <Image src={assets.logo} alt='logo' priority={true} className="w-28 curser-pointer mr-14" />
        </a>
        ...
        <div className='flex items-center gap-4'>
          ...
          <button className='bl ock md:hidden ml-3' onClick={openMenu} >
            <Image src={assets.menu_black} alt="menu icon" priority={true} className='w-6' />
          </button>
        </div>
        {/* ------------mobile menu------------ */}
        <ul ref={sideMenuRef} className='flex md:hidden flex-col gap-4 py-20 px-20 px-10 fixed -right-64 top-0 bottom-0 w-64 z-50 h-screen bg-rose-50 transition duration-500'>
